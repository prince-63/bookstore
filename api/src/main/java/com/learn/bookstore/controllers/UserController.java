...
public class UserController {
    ...
    @Operation(summary = "Delete user by email", description = "Delete user by there email.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "User deleted successfully"),
            @ApiResponse(responseCode = "401", description = "User not authenticated"),
            @ApiResponse(responseCode = "500", description = "Internal server error", content = @Content(schema = @Schema(implementation = ErrorResponseDTO.class)))
    })
    @DeleteMapping(UserEndPointsConstants.DELETE_USER_BY_EMAIL)
    public ResponseEntity<ResponseDTO<String>> deleteUserByEmail(
        ...
        response.setSuccess(true);
        response.setMessage("User deleted successfully.");
        response.setData(String.format("User deleted successfully: %s", email));
        return ResponseEntity.status(HttpStatus.OK).body(response);
    }
